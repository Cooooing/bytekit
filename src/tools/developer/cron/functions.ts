...
function parseField(field: string, min: number, max: number): number[] {
	...
	for (const part of parts) {
		if (part === '*') {
			...
			const s = start === '*' ? min : parseInt(start, 10);
...
function buildDescription(parts: string[]): string {
	...
	if (minute === '*' && hour === '*') desc.push('每分钟');
	else if (minute === '0' && hour === '*') desc.push('每小时的第 0 分钟');
	else if (minute !== '*' && hour === '*') desc.push(`每小时的第 ${minute} 分钟`);
	else desc.push(`${hour}:${minute.padStart(2, '0')}`);
...
export function parseCron(expression: string): { ok: true; result: CronResult } | { ok: false; error: string } {
	...
	if (parts.length !== 5) {
		return { ok: false, error: 'Cron 表达式必须包含 5 个字段：分 时 日 月 周' };
	}
	try {
		const [minute, hour, dayOfMonth, month, dayOfWeek] = parts;
