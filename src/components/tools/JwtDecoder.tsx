...
export default function JwtDecoder() {
	...
	return (
		<IoWorkbench
			...
			output={<CodeEditor title={text.result} value={output} language="json" status={isEmpty ? 'neutral' : result.ok ? 'success' : 'error'} statusText={isEmpty ? text.waiting : result.ok ? text.json : text.fail} message={isEmpty ? text.emptyMessage : undefined} error={result.ok ? undefined : result.error} />}
		/>
	);
}
